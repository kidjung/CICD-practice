pipeline {
    ...
    stages { 
        ...
        stage('Docker run') { 
            agent any 
            steps { 
                // sh 'docker ps -f name=web -q \ | xargs --no-run-if-empty docker container stop' 
                // sh 'docker ps -f name=server -q \ | xargs --no-run-if-empty docker container stop' 
                // sh 'docker container ls -a -f name=web -q \ | xargs -r docker container rm' 
                // sh 'docker container ls -a -f name=server -q \ | xargs -r docker container rm' 
